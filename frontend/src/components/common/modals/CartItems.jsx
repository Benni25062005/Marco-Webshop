    ...
    <>
      <div className="bg-white shadow-md rounded-xl p-6">
        ...
        <div className="space-y-6">
          {cartItems.map((item) => (
            <div
              key={`${item.product_id || item.idProdukt}_${
                item.Name || Math.random()
              }`}
              className="flex justify-between items-center bg-white shadow-md rounded-xl p-4 border"
            >
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2 border border-gray-300 rounded-xl px-2 py-1">
                  <button
                    onClick={() =>
                      onUpdateQuantity(
                        item.product_id || item.idProdukt,
                        item.menge - 1
                      )
                    }
                  >
                ...
                <p className="whitespace-nowrap font-medium">
                  CHF {Number(item.Preis_brutto).toFixed(2)}
                </p>
                <Trash2
